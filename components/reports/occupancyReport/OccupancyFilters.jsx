...
export function OccupancyFilters({ startDate, endDate, roomType, roomTypes, onChangeStart, onChangeEnd, onChangeRoomType, onReset }) {
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-slate-100 p-4 flex flex-col md:flex-row md:items-end gap-3">
      <div className="flex-1">
        ...
        <p className="text-[11px] text-slate-300">Date range and room type for dynamic occupancy insights.</p>
      </div>
      <div className="flex flex-wrap gap-3 items-end">
        ...
        {roomTypes && roomTypes.length > 0 && (
          <div className="space-y-1">
            <Label className="text-[11px] text-slate-200">Room Type</Label>
            <select
              className="h-8 text-xs rounded-md border border-slate-600 bg-slate-900/40 px-2 min-w-[120px]"
              value={roomType || ''}
              onChange={(e) => onChangeRoomType?.(e.target.value || null)}
            >
