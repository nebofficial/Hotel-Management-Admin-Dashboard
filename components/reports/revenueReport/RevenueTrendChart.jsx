...
export function RevenueTrendChart({ trend = [], departmentRevenue = [], loading }) {
  const lineData = (trend || []).map((d) => ({
    date: d.date?.slice(5) || d.date,
    total: d.total ?? 0,
    Rooms: d.Rooms ?? 0,
    Restaurant: d.Restaurant ?? 0,
  }))
  const pieData = (departmentRevenue || []).filter((d) => (d.value || 0) > 0)
    ...
    <div className="space-y-4">
          ...
          {loading || !lineData.length ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={lineData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="Rooms" stroke="#8b5cf6" strokeWidth={1.5} dot={false} strokeDasharray="4 4" name="Rooms" />
                <Line type="monotone" dataKey="Restaurant" stroke="#f59e0b" strokeWidth={1.5} dot={false} strokeDasharray="4 4" name="Restaurant" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </CardContent>
      ...
      {pieData.length > 0 && (
        <Card className="rounded-2xl border border-slate-200 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-slate-800">Revenue Distribution</CardTitle>
          </CardHeader>
      ...
      {pieData.length > 0 && (
        <Card className="rounded-2xl border border-slate-200 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-slate-800">Department Revenue</CardTitle>
          </CardHeader>
          <CardContent className="h-48">
            <ResponsiveContainer width="100%" height="100%">
