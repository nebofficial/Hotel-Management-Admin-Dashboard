...
export function SystemChangeLogs({ logs = [], loading }) {
  return (
    <Card className="rounded-2xl border-0 bg-gradient-to-br from-purple-500/10 via-violet-500/5 to-fuchsia-500/10 shadow-sm overflow-hidden">
      <CardHeader className="pb-2 pt-4 px-4 border-b border-purple-200/50">
        <CardTitle className="text-sm font-semibold text-slate-800">System Changes Tracking</CardTitle>
        <p className="text-[11px] text-slate-500">Configuration and role / permission changes.</p>
      </CardHeader>
      <CardContent className="p-0 overflow-x-auto">
        {loading ? (
          <p className="text-xs text-slate-500 py-8 text-center">Loading...</p>
        ) : !logs.length ? (
          <p className="text-xs text-slate-500 py-8 text-center">No system changes in range</p>
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-purple-100/80 border-b border-purple-200/60">
            ...
            <tbody>
              {logs.slice(0, 50).map((log) => (
                <tr key={log.id} className="border-b border-slate-100 hover:bg-purple-50/40">
