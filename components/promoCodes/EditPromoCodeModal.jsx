...
export function EditPromoCodeModal({ open, promo, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    setMinOrderValue(promo.minOrderValue != null ? String(promo.minOrderValue) : '')
    ...
    setRoomTypes(promo.roomTypes || [])
    setError(null)
  }, [promo])

  const handleSave = async () => {
    if (!promo || !code || !name || !discountValue) return
    setSaving(true)
    try {
      setError(null)
      await onSave?.(promo.id, {
      ...
      console.error('Edit promo modal error', err)
  ...
  return (
    <Dialog open={open} onOpenChange={(openVal) => !openVal && onClose?.()}>
      <DialogContent className="max-w-lg bg-gradient-to-br from-purple-600 via-violet-600 to-indigo-700 text-slate-50 border-0">
        <DialogHeader>
          <DialogTitle className="text-sm font-semibold text-white">Edit Promo Code</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button
            type="button"
            disabled={!promo || !code || !name || !discountValue || saving}
            onClick={handleSave}
