...
export function EditSeasonRuleModal({ open, rule, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    setRoomTypes((rule.roomTypes || []).join(', '))
    setAdjustmentPercent(
      rule.adjustmentPercent != null ? String(rule.adjustmentPercent) : '',
    )
    setAdjustmentType(rule.adjustmentType || 'increase')
    setRuleType(rule.ruleType || 'season')
    setError(null)
  }, [rule])

  const handleSave = async () => {
    if (!rule || !name || !startDate || !endDate || !adjustmentPercent) return
    setSaving(true)
    try {
      setError(null)
      ...
      console.error('Edit season rule modal error', err)
      setError(err?.message || 'Failed to update seasonal pricing rule')
    } finally {
      setSaving(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={(openVal) => !openVal && onClose?.()}>
      <DialogContent className="max-w-lg bg-gradient-to-br from-purple-600 via-violet-600 to-indigo-700 text-slate-50 border-0">
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button
            type="button"
            disabled={!rule || !name || !startDate || !endDate || !adjustmentPercent || saving}
            onClick={handleSave}
