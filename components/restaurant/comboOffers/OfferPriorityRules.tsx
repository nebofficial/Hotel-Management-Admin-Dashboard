...
export default function OfferPriorityRules({ discounts, onRefresh }: Props) {
  const { user } = useAuth()
    ...
    <section className="space-y-3">
      <Card className="rounded-2xl bg-linear-to-r from-slate-500 to-gray-500 text-white shadow-sm border-none">
        <CardContent className="p-3 flex items-center justify-between">
          <div>
            <div className="text-[11px] uppercase tracking-wide opacity-80">Priority rules</div>
            <div className="text-lg font-semibold">{sortedDiscounts.length.toString().padStart(2, "0")}</div>
          </div>
          <ArrowUpDown className="h-8 w-8 opacity-80" />
        </CardContent>
      </Card>
      ...
      <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <CardHeader className="pb-2 pt-3 px-3">
          <CardTitle className="text-sm font-semibold text-slate-900">Offer priority rules (Highest discount / Manual priority)</CardTitle>
        </CardHeader>
        <CardContent className="p-3 pt-0">
          <div className="space-y-2">
            {sortedDiscounts.length === 0 ? (
              <div className="py-8 text-center text-xs text-slate-400">No discount offers to prioritize.</div>
            ) : (
              sortedDiscounts.map((offer, index) => (
          ...
          <DialogContent className="sm:max-w-md" showCloseButton>
            ...
            <div className="grid grid-cols-1 gap-3 pt-1">
              <div className="space-y-1">
                <label className="text-[11px] font-medium text-slate-700">Priority (higher = applied first)</label>
                <Input type="number" value={priority} onChange={(e) => setPriority(e.target.value)} placeholder="0" className="h-8 text-xs" />
              </div>
              <div className="text-xs text-slate-500">
                Higher priority offers are applied first when multiple offers are eligible.
              </div>
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setEditing(null)}>Cancel</Button>
                <Button size="sm" className="h-8 text-xs bg-slate-600 hover:bg-slate-700" disabled={saving} onClick={handleUpdate}>
                  {saving ? "Saving..." : "Save"}
                </Button>
              </div>
            </div>
